...
//                    set_header_bottom();
		...
		jQuery("a.wd-prettyPhoto").prettyPhoto({
			...
			/*,changepicturecallback : function(){
				jQuery('.wd_feedback').find('input.wpcf7-submit').live('click',function(){
					var temp_ctf = 0;
					jQuery('input.wpcf7-validates-as-required').each(function(i,value){
						if(jQuery(value).val() == ''){
							temp_ctf = 1;
							return;
						}
					...
					if(temp_ctf == 1) {
						jQuery('.wd_feedback').find('ul.wd_ctf_error').remove();
						jQuery('.wd_feedback').find('form.wpcf7-form').after( $( "<ul class=\'wd_ctf_error woocommerce-error\'><li>Please fill the required field.</li></ul>" ) );
						return false;
					}
				});
			}*/
